 ...
 //   phist = 0;
  ...
  //compute the prediction
   // static bool pred_taken;
    Perceptron *Q = Perceptron_new(3,0.5);
    ...
    if (brtype == OPTYPE_CALL_DIRECT_COND) {
      ...
      HitBank = AltBank = -1;
      //HitPred = AltPred = btable.predict(result);
      HitPred = AltPred = gshare.predict(compresult[0]);
      //HitPred = AltPred = gselect.predict(result);
      //HitPred = AltPred = gag.predict(result);
      for (int i=0; i<NHIST; i++) {
    ...
    //bool X[] = {TagePred, SCPred, loopPred};
      ...
      if (HitBank >= 0) {
        ...
        if ((gtable[HitBank][GI[HitBank]].u.read() == 0)) {
          if (AltBank >= 0) {
            ...
            //btable.update(result, taken);
            gshare.update(result[0],taken);
            //gselect.update(result,taken);
            //gag.update(result,taken);
          }
        }
      } else {
        //btable.update(result, taken);
