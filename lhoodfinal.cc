...
bool   PREDICTOR::GetPrediction(UINT32 PC, bool a, bool b, bool c){
...
void  PREDICTOR::UpdatePredictor(UINT32 PC, OpType opType, bool resolveDir, bool predDir, UINT32 branchTarget, bool a, bool b, bool c){
  tage->update(PC & 0x3ffff, OPTYPE_CALL_DIRECT_COND, resolveDir, branchTarget & 0x7f);
 // bool X[3] = {SCPred, TagePred, loopPred};
  //Perceptron_getResult(Q,X);
  //Perceptron_train(Q,X,resolveDir);
...
void    PREDICTOR::TrackOtherInst(UINT32 PC, OpType opType, bool predDir, UINT32 branchTarget){
  // Decode instruction
