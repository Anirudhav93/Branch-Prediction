...
#include <inttypes.h>
#include <math.h>
static uint32_t phist = 0;
static bool result[3] ;
static bool pred_taken = true;
static bool Taken;
static uint32_t compresult[3];
//static bool Taken ;
//static bool a;
static int maxconf =0;
static int bselect[4]= {0,0,0,0};
static int A = 0;
static int LS[16][16];
#ifndef PERCEPTRON_H
#define PERCEPTRON_H
...
#define NSTEP 4
#define NDIFF 3
#define NALLOC 7//5,4,5,6,7
#define NHIST 20//20
#define NSTAT 5
#define MSTAT 7//5,6,7
...
//Perceptron *Q = Perceptron_new(3,0.3);
//bool X[3] = {SCPred, TagePred, loopPred};
...
public:
  int init() {
    ...
    compresult[0] = pc^phist;
    result[0] = pred[getIndex(compresult[0])];
    return pred[getIndex(compresult[0])];

  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(compresult[0])] << 1) + hyst[getIndex(compresult[0], HSFT)];
    ...
    if(taken == pred[getIndex(compresult[0])])
        bselect[0]++;
    else
        bselect[0]--;
    if(maxconf == 0)
    {
    pred[getIndex(compresult[0])] = (inter >= 2);

    hyst[getIndex(compresult[0],HSFT)] = ((inter & 1)==1);
    }
  }
};
...
public:
  int init() {
    ...
    compresult[1] = pc;
    result[1] = pred[getIndex(compresult[1])];
    return pred[getIndex(compresult[1])];

  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(compresult[1])] << 1) + hyst[getIndex(compresult[1], HSFT)];
    ...
    if(taken == pred[getIndex(compresult[1])])
      bselect[1]++;
    else
      bselect[1]--;
    if(maxconf ==1)
    {
    pred[getIndex(compresult[1])] = (inter >= 2);

    hyst[getIndex(compresult[1],HSFT)] = ((inter & 1)==1);
    }
  }
};
...
public:
  int init() {
    ...
    compresult[2] = pc+phist;
    result[2] = pred[getIndex(compresult[2])];
    return pred[getIndex(compresult[2])];

  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(compresult[2])] << 1) + hyst[getIndex(compresult[2], HSFT)];
    ...
    if(taken == pred[getIndex(compresult[2])])
      bselect[2]++;
    else
      bselect[2]--;
    if(maxconf ==2 )
    {
    pred[getIndex(compresult[2])] = (inter >= 2);

    hyst[getIndex(compresult[2],HSFT)] = ((inter & 1)==1);
    }
  }
};
...
public:
  int init() {
    ...
    compresult[3] = phist;
    result[3] = pred[compresult[3]];
    return pred[getIndex(compresult[3])];

  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(compresult[3])] << 1) + hyst[getIndex(compresult[3], HSFT)];
    ...
    if (taken == pred[getIndex(compresult[3])])
      bselect[3]++;
    else
      bselect[3]--;
    if(maxconf ==3)
    {
    pred[getIndex(compresult[3])] = (inter >= 2);

    hyst[getIndex(compresult[3],HSFT)] = ((inter & 1)==1);
    }
  }


bool lselect(bool taken)

{

   A = result[0]*1+ result[1]*2 + result[2]*4+result[3]*8;
   LS[A][0]++;
	  if(taken)
		LS[A][1]++;

		if (LS[A][1]/LS[A][0]>0.5)
		return true;
...
class my_predictor {
  ...
  /////////////////////////////////////////////////////////
  //uint32_t phist; //path history
  // Prediction Tables

  Bimodal<LOGB,HYSTSHIFT> btable; // bimodal table
  Gshare<LOGB,HYSTSHIFT> gshare;//gshare table
  Gselect<LOGB,HYSTSHIFT> gselect; //gselect table
  Gag<LOGB,HYSTSHIFT> gag;  //Gag table
...
public:
  ...
  my_predictor (void) {
    ...
    logg[STEP[0]] = LOGG ;
    logg[STEP[1]] = LOGG ;
    logg[STEP[2]] = LOGG + 6; //3
    logg[STEP[3]] = LOGG -3;//-1
    TB[STEP[0]] =  7;
    TB[STEP[1]] =  9;
    TB[STEP[2]] = 11;
    TB[STEP[3]] = 13;
 ...
 //   phist = 0;
  ...
  //compute the prediction
   // static bool pred_taken;
    Perceptron *Q = Perceptron_new(4,0.5);
    //bool X[3] = {TagePred, SCPred, loopPred};
    int k=0;
   ...
   //gag.maxiconf();
      ...
      //  if (maxconf == 0)
      //  HitPred = AltPred = gshare.predict(pc);
      //
      //  else if (maxconf ==1)
      //  HitPred = AltPred = btable.predict(pc);
      //
      //  else if(maxconf ==2)
      //  HitPred = AltPred = gselect.predict(pc);
      //
      //  else
      //  HitPred = AltPred = gag.predict(pc);

			HitPred = AltPred = gag.lselect(Taken);
    ...
    bool X[] = {result[0], result[1], result[2], result[4]};

		//pred_taken = Perceptron_getResult(Q,X);
    Perceptron_train(Q,X,Taken);
    ...
    if (brtype == OPTYPE_CALL_DIRECT_COND) {
      ...
      if (HitBank >= 0) {
        ...
        if ((gtable[HitBank][GI[HitBank]].u.read() == 0)) {
          ...
          //if(bselect == 0)
            gshare.update(result[0],taken);

          //else if (bselect ==1)
            btable.update(result[1], taken);

          //else if (bselect ==2)
            gselect.update(result[2],taken);

          //else
            gag.update(result[3],taken);
          }
        }
      } else {
...
class PREDICTOR{
 ...
 public:
  ...
  //my_predictor *tage;
  PREDICTOR(void);
  bool    GetPrediction(UINT32 PC, bool a, bool b, bool c);
  void    UpdatePredictor(UINT32 PC, OpType opType, bool resolveDir, bool predDir, UINT32 branchTarget, bool a, bool b, bool c);
