...
public:
  int init() {
    ...
    compresult = pc^phist;
    result = compresult;
    return pred[getIndex(compresult)];

  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(compresult)] << 1) + hyst[getIndex(compresult, HSFT)];
    ...
    pred[getIndex(compresult)] = (inter >= 2);

    hyst[getIndex(compresult,HSFT)] = ((inter & 1)==1);
  ...
  //compute the prediction
   // static bool pred_taken;
    ...
    if (brtype == OPTYPE_CALL_DIRECT_COND) {
      ...
      HitPred = AltPred = gshare.predict(result);
