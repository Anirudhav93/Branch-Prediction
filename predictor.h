...
private:
  ...
  int mask ;
  uint32_t phtIndex=0x00;
  //uint32_t *check[MAXHIST];
  bool check[MAXHIST];
...
public:
  ...
  bool predict(uint32_t pc) {
    ...
    //return pred[getIndex(phtIndex)];
  }

  void update(uint32_t pc, bool taken) {
    int inter = (pred[getIndex(pc)] << 1) + hyst[getIndex(pc, HSFT)];
    //int inter = (pred[getIndex(phtIndex)]<<1)+hyst[phtIndex];
    if(taken) {
      if (inter < 3) { inter++; }
    } else {
      if (inter > 0) { inter--; }
    ...
    //pred[getIndex(phtIndex)] = (inter>=2);
    hyst[getIndex(pc,HSFT)] = ((inter & 1)==1);
    //hyst[getIndex(phtIndex,HSFT)] = ((inter&1)==1);
  }
};
...
public:
  ...
  void update(uint32_t pc, uint16_t brtype, bool taken, uint32_t target) {
    if (brtype == OPTYPE_CALL_DIRECT_COND) {
      ...
      const double X[] = {loopPred,SCPred,TagePred};
			Perceptron *p = Perceptron_new(3,10);
